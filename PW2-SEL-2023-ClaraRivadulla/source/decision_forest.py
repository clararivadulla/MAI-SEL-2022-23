...
class DecisionTree:
    ...
    def grow_tree(self, X, y, depth=0):
        ...
        if depth >= self.max_depth or self.gini(y) < self.min_impurity:
            print(y)
            most_common_pred_class = np.unique(y, return_counts=True)[0][np.unique(y, return_counts=True)[1].argmax()]
            print(most_common_pred_class)
            return Node(pred_class=most_common_pred_class) # Return the most common class
